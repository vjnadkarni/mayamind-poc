...
**Version 1.06 | February 2026 | CONFIDENTIAL**
...
Cloud dependencies are limited to two API services: the conversational LLM (Claude API, including native web search at $0.01/search) and text-to-speech (ElevenLabs). This architecture keeps per-user cloud costs low while maintaining high-quality conversational and voice experiences.
...
| ID | Feature | Description | Pri | Phase |
|----|---------|-------------|-----|-------|
| PC-01 | Interest profiling | During onboarding and ongoing conversation, build a detailed profile of user's interests using a progressive three-tier consent model (Session Only → Preferences + Topics → Full Personality). Use to drive content recommendations and personalized responses. Profile stored locally on iPad with full user transparency via voice-accessible "What do you know about me?" command. | P0 | Pilot |
...
| DA-03 | Weather and day planning | Proactively share weather information and suggest activities appropriate for conditions. Uses real-time web search to provide accurate, timezone-aware weather data for the user's location. | P0 | Pilot |
| DA-04 | General knowledge Q&A | Answer factual questions — health information, how-to, historical facts — with appropriate caveats for medical questions. For current events, news, and sports scores, the companion uses real-time web search to provide accurate, up-to-date answers. | P0 | Pilot |
...
- **Web search:** Claude native web search (Brave Search, server-side connector) for real-time information — weather, news, sports scores, current events. Billed at $0.01/search through the Claude API; no separate API keys required.
- **Text-to-speech:** ElevenLabs API for natural voice synthesis with word-level timestamps for lip-sync. Text is sanitized before synthesis — symbols (degrees, percent, inches) are converted to spoken words for natural pronunciation.
- **Content services:** News, recipes, weather, and other content APIs. Most real-time information needs are now served by Claude's native web search.
...
| Web Search | Claude native search (Brave) | Cloud, $0.01/search via Claude API |
...
- **Claude Web Search:** $0.01 per search, billed through Claude API. Usage assumption: ~5–15 searches per day per user (weather, news, sports, general knowledge). Estimated cost: $0.05–$0.15/day per user.
...
- **Content APIs:** Most real-time information needs (news, weather, sports) are now served by Claude's native web search, reducing or eliminating the need for separate content APIs. Recipe APIs may still be used for specialized content.
...
| v1.06 | Feb 26, 2026 | Vijay / Claude | Added real-time web search via Claude native search tool (DA-03, DA-04 elevated to P0). Added progressive three-tier personalization system to PC-01. Added web search and TTS text sanitization to technology stack (Sec 12.3) and cloud services (Sec 9.3). Updated cloud pricing analysis (Sec 15) with web search cost estimates. Companion can now answer weather, news, sports, and current events questions with timezone-aware, up-to-date information. Validated in working POC. |
