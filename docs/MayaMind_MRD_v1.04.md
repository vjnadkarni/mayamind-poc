...
- **Real-time information access.** The companion answers questions about weather, news, sports scores, and current events through integrated web search — making it genuinely useful for daily information needs. Competitors like ElliQ and Meela do not offer real-time search-backed responses.
- **Progressive personalization.** A three-tier, consent-based personalization system where the companion learns the user's name, preferences, interests, and communication style over time. Users have full transparency and control over what is remembered, with a voice-accessible "What do you know about me?" command that shows exactly what the system has learned.
...
- **Low cloud costs.** On-device avatar rendering and speech recognition eliminate the two most expensive cloud dependencies that competitors face. Only LLM inference and text-to-speech require cloud APIs. Web search is integrated natively into the LLM at $0.01/search.
...
| v1.04 | February 26, 2026 | Vijay / Claude | Added real-time information access and progressive personalization to competitive differentiation (Sec 5.2). Validated web search, timezone-aware responses, consent-based personalization, and TTS text sanitization in working POC. |
